...
function getUserId() {
  try {
    const app = getApp();
    // cloud 模式：优先使用 userInfo 中的 openid
    if (config.mode === 'cloud' && app && app.globalData) {
      const userInfo = app.globalData.userInfo;
      if (userInfo && userInfo.openid) return userInfo.openid;
    }
    // 使用 token 前 16 位作为匿名标识
...
function flush() {
  ...
  if (config.mode === 'cloud') {
    ...
    const app = getApp();
    const cloudOk = app && app.globalData && app.globalData.cloudReady && app.globalData.cloudAvailable;
    if (!cloudOk) {
      // Cloud not ready or previously timed out — drop logs silently
      return;
    }
      ...
      fail: (err) => {
        if (err.errMsg && String(err.errMsg).includes('timeout')) {
          ...
          if (app && app.globalData) {
            app.globalData.cloudAvailable = false;
          }
        }
      },
    });
